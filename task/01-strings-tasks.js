...
function encodeToRot13(str) {
    const ARRSOURCE = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.split('');
    const ARRRES = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.split('');
    let arr = str.split('');
    for (let i = 0; i < str.length; i++) {
        for (let j = 0; j < ARRSOURCE.length; j++) {
            if (arr[i] == ARRSOURCE[j]) {
				arr[i] = ARRRES[j];
				break;
            }
        }
...
function isString(value) {
    //return typeof(value) == 'string' || value.fromCharCode();
    throw new Error('Not implemented');
}
